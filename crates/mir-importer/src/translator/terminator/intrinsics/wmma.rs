...
use dialect_nvvm::ops::{
    ...
    MmaM16N8K32F32E4m3Op, MmaM16N8K32S32S8Op, MovmatrixTransB16Op,
};
...
/// Emit `mma_m16n8k32_f32_e4m3` as a register-producing dialect operation.
///
/// Args:
/// - `args[0]`: `[f32; 4]` C accumulator registers
/// - `args[1]`: `[u32; 4]` packed A fragment registers
...
///
/// Returns: `[f32; 4]` D accumulator registers.
pub fn emit_mma_m16n8k32_f32_e4m3(
    ctx: &mut Context,
    body: &mir::Body,
    args: &[mir::Operand],
    destination: &mir::Place,
    ...
    if args.len() != 3 {
        return input_err!(
            loc.clone(),
            TranslationErr::unsupported(format!(
                "mma_m16n8k32_f32_e4m3 expects 3 arguments (acc, a, b), got {}",
                args.len()
            ))
        );
    }
    ...
    let mma_op = Operation::new(
        ctx,
        MmaM16N8K32F32E4m3Op::get_concrete_op_info(),
        vec![f32_ty.into(); 4],
        operands,
        vec![],
        0,
    ...
    emit_store_result_and_goto(
        ...
        block_map,
        loc,
        "mma_m16n8k32_f32_e4m3 call without target block",
    )
}
