...
/// Register-only warp MMA: m16n8k32 with f32 accumulator and f16 inputs.
///
/// # Operands
///
/// - operands 0-3: four f32 C accumulator registers
...
/// - results 0-3: four f32 D accumulator registers
#[pliron_op(
    name = "nvvm.mma_m16n8k32_f32_e4m3",
    format,
    interfaces = [NOpdsInterface<10>, NResultsInterface<4>],
)]
pub struct MmaM16N8K32F32E4m3Op;

impl Verify for MmaM16N8K32F32E4m3Op {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
        let op = self.get_operation().deref(ctx);
        let operands: Vec<_> = op.operands().collect();
        ...
        if operands.len() != 10 {
            return verify_err!(
                op.loc(),
                "nvvm.mma_m16n8k32_f32_e4m3 requires 10 register operands, got {}",
                operands.len()
            );
        }
        ...
        for (index, operand) in operands.iter().take(4).enumerate() {
            ...
            if ty.deref(ctx).downcast_ref::<FP32Type>().is_none() {
                return verify_err!(
                    op.loc(),
                    "nvvm.mma_m16n8k32_f32_e4m3 C operand {} must be f32",
                    index
                );
            }
        }
        ...
        for (index, operand) in operands.iter().enumerate().skip(4) {
            ...
            let Some(integer) = ty.downcast_ref::<IntegerType>() else {
                return verify_err!(
                    op.loc(),
                    "nvvm.mma_m16n8k32_f32_e4m3 packed operand {} must be i32",
                    index
                );
            };
            if integer.width() != 32 {
        ...
        if op.get_num_results() != 4 {
            return verify_err!(
                op.loc(),
                "nvvm.mma_m16n8k32_f32_e4m3 requires 4 f32 results"
            );
        }
        ...
        for index in 0..4 {
            ...
            if ty.deref(ctx).downcast_ref::<FP32Type>().is_none() {
                return verify_err!(
                    op.loc(),
                    "nvvm.mma_m16n8k32_f32_e4m3 result {} must be f32",
                    index
                );
            }
        }
...
pub(super) fn register(ctx: &mut Context) {
    ...
    MmaM16N8K32F32E4m3Op::register(ctx);
