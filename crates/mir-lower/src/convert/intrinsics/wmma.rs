...
/// Convert `mma_m16n8k32_f32_e4m3` to one register-only inline PTX operation.
///
/// Operand order is C[0..4], A[0..4], B[0..2]. The four D registers are
/// returned as an LLVM struct and then split back into the dialect op's four
/// SSA results. There are no hidden pointer, stack, load, or store operands.
pub(crate) fn convert_mma_m16n8k32_f32_e4m3(
    ctx: &mut Context,
    rewriter: &mut DialectConversionRewriter,
    op: Ptr<Operation>,
    _operands_info: &OperandsInfo,
    ...
    if operands.len() != 10 {
        return pliron::input_err_noloc!(
            "mma_m16n8k32_f32_e4m3 requires 10 register operands, got {}",
            operands.len()
        );
    }
